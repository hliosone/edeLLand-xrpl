/**
 * POST /api/loans/collateral/finalize
 *
 * Step 5 of the collateral loan flow.
 * Receives the borrower-signed LoanSet blob, broker countersigns,
 * submits to XRPL, then links the resulting loanId to the collateral position.
 *
 * Body:    { signedBlob: string, borrowerAddress: string, loanRequestId: string }
 * Returns: { ok, hash, loanId, periodicPayment, nextPaymentDueDate, paymentsTotal,
 *            collateralXRP, xrpDropsLocked }
 */

import { NextResponse }                     from "next/server";
import { Client, Wallet, signLoanSetByCounterparty } from "xrpl";
import { getById, activateLoan }            from "../../../../../lib/collateral-store.js";
...
export async function POST(request) {
  try {
    const { signedBlob, borrowerAddress, loanRequestId } = await request.json();

    if (!signedBlob || !borrowerAddress || !loanRequestId) {
      return NextResponse.json(
        { error: "signedBlob, borrowerAddress and loanRequestId required" },
        { status: 400 }
      );
    }

    // ── 1. Verify collateral position ────────────────────────────────────────
    ...
    if (position.status !== "deposit_confirmed") {
      return NextResponse.json(
        { error: `Cannot finalise: collateral status is '${position.status}', expected 'deposit_confirmed'.` },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      if (!types.has(CRED_KYC_FULL) && !types.has(CRED_KYC_OVER18)) {
        return NextResponse.json({ error: "KYC credential missing." }, { status: 403 });
      }
      ...
      // ── 5. Activate collateral position ──────────────────────────────────
      if (loanId) activateLoan(loanRequestId, loanId);

      const loanScale          = loan?.LoanScale ?? -6;
      const periodicPaymentRaw = loan ? parseFloat(loan.PeriodicPayment) : null;
      const xrpDropsLocked     = position.xrpDrops;
      ...
      return NextResponse.json({
        ...
        collateralXRP:    (parseInt(xrpDropsLocked) / 1_000_000).toFixed(6),
        xrpDropsLocked,
      });
    } finally {
    ...
    console.error("[collateral/finalize]", err);
